...
def report(R, per, tag):
    ...
    print("="*58)
    if R.get('matched',0)==0 or R['pos']<=0:
        print("  No fused tracks matched the truth  -  nothing to score.\n")
        print("  verify.py scores REPLAY mode ONLY (it compares against truth_s3.npy).")
        print("  Feed the board with:   python sensor_sim.py --board 192.168.1.10")
        print("  (i.e. WITHOUT --live).  Live mode tracks real aircraft, which have")
        print("  no ground truth to compare against, so RMSE cannot be computed.\n")
        print("="*58)
        return

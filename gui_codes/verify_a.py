...
# def score(frames, truth):
#     acc = dict(pos=0.0, rng=0.0, az=0.0, el=0.0, vel=0.0, n=0, active=0)
#     per=[]
#     for epoch, tracks in frames:
#         if epoch<10 or epoch>=truth.shape[0]: continue
#         tp = truth[epoch]; mask=~np.isnan(tp[:,0]); act=tp[mask]
...
#     print("\n"+"="*58)
#     print(f"  ON-HARDWARE VERIFICATION  ({tag})")
#     print("="*58)
...
#         act=truth[e][~np.isnan(truth[e][:,0])]; tr=[]
#         for g in act:
#             if rng.random()<0.97:
#                 pos=g[:3]+rng.normal(0,15,3); vel=g[3:6]+rng.normal(0,8,3)   # ~15 m/axis, ~8 m/s
#                 tr.append((pos[0],pos[1],pos[2],vel[0],vel[1],vel[2]))
#         frames.append((e,tr))
#     return frames
...
#         frames=selftest(truth); tag="SELF-TEST (synthetic)"
#     else:
#         frames=collect_from_board(); tag="live board capture"
#     R,per=score(frames,truth); report(R,per,tag)
